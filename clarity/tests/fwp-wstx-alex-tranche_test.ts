...
Clarinet.test({
  name: "fwp-wstx-alex-tranched-120 : ensure that contract is activated when adding to position",
  async fn(chain: Chain, accounts: Map<string, Account>) {
...
Clarinet.test({
  name: "fwp-wstx-alex-tranched-120 : ensure that stacking is available when adding to position",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get("deployer")!;
    const wallet_1 = accounts.get("wallet_1")!;
    const reservePool = new ReservePool(chain);
...
Clarinet.test({
  name: "fwp-wstx-alex-tranched-120 : ensure that add-to-position works on a valid pool",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get("deployer")!;
    const wallet_1 = accounts.get("wallet_1")!;
    ...
    block = chain.mineBlock([Tx.contractCall("fwp-wstx-alex-tranched-120", "add-to-position", [ types.uint(dx) ], wallet_1.address),]);
    ...
    block.receipts[0].events.expectFungibleTokenTransferEvent(
      dx,
      wallet_1.address,
      deployer.address + ".fwp-wstx-alex-tranched-120",
      "fwp-wstx-alex-50-50-v1-01"
    );
    block.receipts[0].events.expectFungibleTokenTransferEvent(
      dx,
    ...
    block = chain.mineBlock([Tx.contractCall("fwp-wstx-alex-tranched-120", "add-to-position", [ types.uint(dx) ], wallet_2.address),]);
    block.receipts[0].result.expectOk().expectBool(true);
    // console.log(block.receipts[0].events);
...
Clarinet.test({
  name: "fwp-wstx-alex-tranched-120 : ensure that claim-and-stake cannot claim future cycles",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get("deployer")!;
    const wallet_1 = accounts.get("wallet_1")!;
    const wallet_2 = accounts.get("wallet_2")!;
...
Clarinet.test({
  name: "fwp-wstx-alex-tranched-120 : ensure that claim-and-stake works with a valid cycle",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get("deployer")!;
    const wallet_1 = accounts.get("wallet_1")!;
    const wallet_2 = accounts.get("wallet_2")!;
...
Clarinet.test({
  name: "fwp-wstx-alex-tranched-120 : ensure that reduce-position works",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get("deployer")!;
    const wallet_1 = accounts.get("wallet_1")!;
    const wallet_2 = accounts.get("wallet_2")!;
...
Clarinet.test({
  name: "fwp-wstx-alex-tranched-120 : ensure that x works",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get("deployer")!;
    const wallet_1 = accounts.get("wallet_1")!;
    const wallet_2 = accounts.get("wallet_2")!;
    const yieldVault = new YieldVault(chain, "fwp-wstx-alex-tranched-120-120");
    const reservePool = new ReservePool(chain);
    ...
    block = chain.mineBlock(
      [
        Tx.contractCall("fwp-wstx-alex-tranched-120-120x", "set-available-alex", 
          [
            types.principal(wallet_1.address), 
            types.uint(dx)
          ], deployer.address
        ),
        Tx.contractCall("fwp-wstx-alex-tranched-120-120x", "add-to-position",
          [
            types.uint(dx)
          ], wallet_1.address
        )
    ...
    block = chain.mineBlock(
      [
        Tx.contractCall("fwp-wstx-alex-tranched-120-120x", "reduce-position", [], wallet_1.address)
      ]
    );
    ...
    block.receipts[0].events.expectFungibleTokenBurnEvent(
      dx,
      deployer.address + ".fwp-wstx-alex-tranched-120-120x",
      "alex"
    );
  },
});
