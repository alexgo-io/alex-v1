...
export function prepareStandardTest(chain: Chain, parameters: StandardTestParameters, deployer: Account) {
	...
	const first = chain.mineBlock([		
		...
		Tx.contractCall("token-apower", "add-approved-contract", [types.principal(contractPrincipal(deployer, "alex-launchpad-v1-2"))], deployer.address),
		...
		Tx.contractCall("alex-launchpad-v1-2", "create-pool", [
	...
	const second = chain.mineBlock([
		Tx.contractCall("alex-launchpad-v1-2", "add-to-position", [types.uint(idoId), types.uint(ticketsForSale), types.principal(contractPrincipal(deployer, "token-wban"))], idoOwner.address),
	]);
