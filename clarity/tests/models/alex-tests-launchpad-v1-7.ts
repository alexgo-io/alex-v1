...
export function prepareStandardTest(chain: Chain, parameters: StandardTestParameters, deployer: Account) {
	...
	const first = chain.mineBlock([		
		...
		Tx.contractCall("token-apower", "add-approved-contract", [types.principal(contractPrincipal(deployer, "alex-launchpad-v1-7"))], deployer.address),
		...
		Tx.contractCall("alex-launchpad-v1-7", "create-pool", [
			...
			types.tuple({
				...
				"total-registration-max": types.uint(totalRegistrationMax)
			}),
		], deployer.address),
	]);
	...
	const second = chain.mineBlock([
		Tx.contractCall("alex-launchpad-v1-7", "add-to-position", [types.uint(idoId), types.uint(ticketsForSale), types.principal(contractPrincipal(deployer, "token-wban"))], idoOwner.address),
	]);
