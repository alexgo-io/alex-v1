import {
  ...
  class CRPYTTestAgent {
    ...
    getPoolDetails(token: string, collateral: string, expiry: number) {
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-pool-details", [
    ...
    getPoolValueInToken(token: string, collateral: string, expiry: number) {
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-pool-value-in-token", [
        types.principal(token),
        types.principal(collateral),
        types.uint(expiry)
      ], this.deployer.address);
    ...
    getPoolValueInCollateral(token: string, collateral: string, expiry: number) {
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-pool-value-in-collateral", [
        types.principal(token),
        types.principal(collateral),
        types.uint(expiry)
      ], this.deployer.address);
    ...
    getWeightY(token: string, collateral: string, expiry: number, strike: number, bs_vol: number) {
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-weight-y", [
        types.principal(token),
        types.principal(collateral),
        types.uint(expiry),
        types.uint(strike),
    ...
    getSpot(token: string, collateral: string){
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-spot", [
        types.principal(token),
        types.principal(collateral)
      ], this.deployer.address);
    }    
    ...
    getLtv(token: string, collateral: string, expiry: number){
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-ltv", [
        types.principal(token),
        types.principal(collateral),
        types.uint(expiry)
      ], this.deployer.address);
    ...
    getTokenGivenPosition(token: string, collateral: string, expiry: number, dx: number){
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-token-given-position", [
        types.principal(token),
        types.principal(collateral),
        types.uint(expiry),
        types.uint(dx)
    ...
    swapXForY(user: Account, token: string, collateral: string, expiry: number, dX: number, dy_min: number) {
      let block = this.chain.mineBlock([
        Tx.contractCall("collateral-rebalancing-pool-yt", "swap-x-for-y", [
          types.principal(token),
          types.principal(collateral),
          types.uint(expiry),
          types.uint(dX),
    ...
    swapYForX(user: Account, token: string, collateral: string, expiry: number, dY: number, min_dx: number) {
        let block = this.chain.mineBlock([
          Tx.contractCall("collateral-rebalancing-pool-yt", "swap-y-for-x", [
            types.principal(token),
            types.principal(collateral),
            types.uint(expiry),
            types.uint(dY),
    ...
    getYgivenX(user: Account, token: string, collateral: string, expiry: number, dX: number) {
      let block = this.chain.mineBlock([
        Tx.contractCall("collateral-rebalancing-pool-yt", "get-y-given-x", [
          types.principal(token),
          types.principal(collateral),
          types.uint(expiry),
          types.uint(dX)
    ...
    getXgivenY(user: Account, token: string, collateral: string, expiry: number, dY: number) {
        let block = this.chain.mineBlock([
          Tx.contractCall("collateral-rebalancing-pool-yt", "get-x-given-y", [
            types.principal(token),
            types.principal(collateral),
            types.uint(expiry),
            types.uint(dY)
    ...
    getBalances(user: Account, token: string, collateral: string, expiry: number) {
        let block = this.chain.mineBlock([
          Tx.contractCall("collateral-rebalancing-pool-yt", "get-balances", [
            types.principal(token),
            types.principal(collateral),
            types.uint(expiry)
          ], user.address),
    ...
    getFeetoAddress(user: Account, token: string, collateral: string, expiry: number) {
      let block = this.chain.mineBlock([
        Tx.contractCall("collateral-rebalancing-pool-yt", "get-fee-to-address", [
          types.principal(token),
          types.principal(collateral),
          types.uint(expiry)
        ], user.address),
    ...
    setFeeRateX(user: Account, token: string, collateral: string, expiry: number, feerate:number) {
      let block = this.chain.mineBlock([
        Tx.contractCall("collateral-rebalancing-pool-yt", "set-fee-rate-x", [
          types.principal(token),
          types.principal(collateral),
          types.uint(expiry),
          types.uint(feerate)
    ...
    setFeeRateY(user: Account, token: string, collateral: string, expiry: number, feerate:number) {
      let block = this.chain.mineBlock([
        Tx.contractCall("collateral-rebalancing-pool-yt", "set-fee-rate-y", [
          types.principal(token),
          types.principal(collateral),
          types.uint(expiry),
          types.uint(feerate)
    ...
    getFeeRateX(user: Account, token: string, collateral: string, expiry: number) {
      let block = this.chain.mineBlock([
        Tx.contractCall("collateral-rebalancing-pool-yt", "get-fee-rate-x", [
          types.principal(token),
          types.principal(collateral),
          types.uint(expiry)
        ], user.address),
    ...
    getFeeRateY(user: Account, token: string, collateral: string, expiry: number) {
      let block = this.chain.mineBlock([
        Tx.contractCall("collateral-rebalancing-pool-yt", "get-fee-rate-y", [
          types.principal(token),
          types.principal(collateral),
          types.uint(expiry)
        ], user.address),
    ...
    getPositionGivenBurnKey(token: string, collateral: string, expiry: number, shares: number) {
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-position-given-burn-key", [
        types.principal(token),
        types.principal(collateral),
        types.uint(expiry),
        types.uint(shares)
    ...
    getXgivenPrice(token: string, collateral: string, expiry: number, price: number) {
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-x-given-price", [
          types.principal(token),
          types.principal(collateral),
          types.uint(expiry),
          types.uint(price)
    ...
    getYgivenPrice(token: string, collateral: string, expiry: number, price: number) {
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-y-given-price", [
        types.principal(token),
        types.principal(collateral),
        types.uint(expiry),
        types.uint(price)
    ...
    setFeeRebate(user: Account, token: string, collateral: string, expiry: number, rebate : number) {
      let block = this.chain.mineBlock([
        Tx.contractCall("collateral-rebalancing-pool-yt", "set-fee-rebate", [
          types.principal(token),
          types.principal(collateral),
          types.uint(expiry),
          types.uint(rebate)
    ...
    getFeeRebate(token: string, collateral: string, expiry: number) {
      return this.chain.callReadOnlyFn("collateral-rebalancing-pool-yt", "get-fee-rebate", [
        types.principal(token),
        types.principal(collateral),
        types.uint(expiry)
      ], this.deployer.address);
