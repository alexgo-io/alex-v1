...
const alexAddress = "ST1HTBVD3JG9C05J7HBJTHGR0GGW7KXW28M5JS8QE." + tokenContract;

class AlexStakingHelper {
    chain: Chain;
    deployer: Account;
    ...
    claimStakingReward(sender: Account, reward_cycle: number) {
    ...
    getStakingRewards(sender: Account, reward_cycle: number) {
      return this.chain.callReadOnlyFn(helperContract, "get-staking-reward-by-tx-sender", [
        types.uint(reward_cycle)
      ], sender.address);
    }  
...
Clarinet.test({
    name: "alex-staking-helper tests",
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        result.expectOk().expectBool(true);
        result = await StakingTest.addToken(deployer, alexAddress);
        result.expectOk().expectBool(true);  
        result = await StakingTest.setCoinbaseAmount(deployer, alexAddress, ONE_8, ONE_8, ONE_8, ONE_8, ONE_8);
        result.expectOk().expectBool(true);      
        result = await StakingTest.registerUser(deployer, alexAddress);
        result.expectOk().expectBool(true);

        result = await StakingTest.stakeTokens(wallet_6, alexAddress, 100e8, 3);
        result.expectOk().expectBool(true);
        ...
        call = await StakingTest.getRewardCycle(alexAddress);
        call.result.expectSome().expectUint(2); 
        
        result = await StakingTest.claimStakingReward(wallet_6, 1);
        ...
        result = await StakingTest.claimStakingReward(wallet_6, 3);
        claimed = result.expectOk().expectTuple();
        claimed['entitled-token'].expectUint(ONE_8);
