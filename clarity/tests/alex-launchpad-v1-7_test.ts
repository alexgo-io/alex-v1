import {
  ...
  extractParameters,
  determineApower,
} from "./models/alex-tests-launchpad-v1-7.ts";
import type {
  Chain,
  Account,
...
const parameters = {
  ...
  totalRegistrationMax: 999999999999
};

Clarinet.test({
  name: "alex-launchpad-v1-7 : only owner can create pool",
    ...
    const block = chain.mineBlock([
      Tx.contractCall(
        ...
        [
          ...
          types.tuple({
            "launch-owner": types.principal(params.idoOwner.address),
            "launch-tokens-per-ticket": types.uint(params.idoTokensPerTicket),
            ...
            "fee-per-ticket-in-fixed": types.uint(params.feePerTicketInFixed),
            "total-registration-max": types.uint(params.totalRegistrationMax)
          }),
        ],
        accountA.address
      ),
...
Clarinet.test({
  name: "alex-launchpad-v1-7 : only launch-owner/approved-address can add-to-positions",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "alex-launchpad-v1-7 : registration is allowed only once",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "alex-launchpad-v1-7 : attempt to register more than max fails",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "alex-launchpad-v1-7 : contract-owner can transfer all tokens in emergency",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
    ...
    block.receipts[2].events.expectSTXTransferEvent(
      ...
      deployer.address + ".alex-launchpad-v1-7",
      deployer.address
    );
...
Clarinet.test({
  name: "alex-launchpad-v1-7 : activation-threshold not reached allows orderly refund",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "alex-launchpad-v1-7 : whitelist works",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "alex-launchpad-v1-7 : example claim walk test",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
