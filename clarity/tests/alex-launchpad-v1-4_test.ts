import {
  ...
  extractParameters,
  determineApower,
} from "./models/alex-tests-launchpad-v1-4.ts";
import type {
  Chain,
  Account,
...
Clarinet.test({
  name: "alex-launchpad-v1-4 : only owner can create pool",
...
Clarinet.test({
  name: "alex-launchpad-v1-4 : only launch-owner/approved-address can add-to-positions",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "alex-launchpad-v1-4 : registration is allowed only once",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "alex-launchpad-v1-4 : attempt to register more than max fails",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "alex-launchpad-v1-4 : contract-owner can transfer all tokens in emergency",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
    ...
    block.receipts[2].events.expectSTXTransferEvent(
      ...
      deployer.address + ".alex-launchpad-v1-4",
      deployer.address
    );
...
Clarinet.test({
  name: "alex-launchpad-v1-4 : activation-threshold not reached allows orderly refund",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
...
Clarinet.test({
  name: "alex-launchpad-v1-4 : example claim walk test",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const [
      deployer,
      accountA,
