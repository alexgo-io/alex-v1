...
(define-public (execute (token <ft-trait>) (amount uint))
    (let
        (   
            ...
            (expiry (unwrap! (contract-call? .yield-usda-74880 get-expiry) ERR-GET-EXPIRY-FAIL-ERR))
            (ltv (try! (contract-call? .collateral-rebalancing-pool get-ltv .token-usda .token-wbtc expiry)))
            (price (try! (contract-call? .yield-token-pool get-price .yield-usda-74880)))
            ...
            (swapped-token (get dx (try! (contract-call? .collateral-rebalancing-pool add-to-position-and-switch .token-usda .token-wbtc .yield-usda-74880 .key-usda-74880-wbtc gross-amount))))            
        )
