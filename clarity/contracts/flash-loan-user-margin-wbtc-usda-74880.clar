...
(define-public (execute-margin-wbtc-usda-74880 (the-amount uint))
...
(define-public (execute)
    (let
        (            
            (swapped-token (get dx (try! (contract-call? .collateral-rebalancing-pool add-to-position-and-switch .token-usda .token-wbtc .yield-usda-74880 .key-usda-74880-wbtc (var-get amount)))))            
        )
        ...
        (print { object: "flash-loan-user-margin-wbtc-usda-74880", action: "execute", data: (var-get amount) })
        (ok true)
    )
)