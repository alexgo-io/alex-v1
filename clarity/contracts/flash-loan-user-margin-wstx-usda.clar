...
(define-public (execute (collateral <ft-trait>) (amount uint) (memo (optional (buff 16))))
    (let
        (   
            ...
            (ltv (try! (contract-call? .collateral-rebalancing-pool-v1 get-ltv .token-wusda .token-wstx memo-uint)))
            (price (try! (contract-call? .yield-token-pool get-price memo-uint .yield-usda)))
            (gross-amount (mul-up amount (div-down price ltv)))            
            (minted-yield-token (get yield-token (try! (contract-call? .collateral-rebalancing-pool-v1 add-to-position .token-wusda .token-wstx memo-uint .yield-usda .key-usda-wstx gross-amount))))
        ...
        (try! (contract-call? .fixed-weight-pool-v1-01 swap-helper .token-wusda .token-wstx u50000000 u50000000 swapped-token none))
