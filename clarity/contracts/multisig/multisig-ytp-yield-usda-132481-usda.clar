...
(define-read-only (is-token-accepted (token <ft-trait>))
    (is-eq (contract-of token) .ytp-yield-usda-132481-usda)
)
...
(define-public (propose
  ...
  (let (
    (proposer-balance (* (unwrap-panic (contract-call? .ytp-yield-usda-132481-usda get-balance tx-sender)) ONE_8))
    (total-supply (* (unwrap-panic (contract-call? .ytp-yield-usda-132481-usda get-total-supply)) ONE_8))
