...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .simple-weight-pool set-fee-rate-x .token-wstx .token-usda new-fee-rate-x)))
    (as-contract (try! (contract-call? .simple-weight-pool set-fee-rate-y .token-wstx .token-usda new-fee-rate-y)))
    (ok true)
  )
)
