...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .fixed-weight-pool-alex set-fee-rate-x .age000-governance-token .token-usda u50000000 u50000000 new-fee-rate-x)))
    (as-contract (try! (contract-call? .fixed-weight-pool-alex set-fee-rate-y .age000-governance-token .token-usda u50000000 u50000000 new-fee-rate-y)))
    (ok true)
  )
)
