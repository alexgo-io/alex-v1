...
(define-constant ERR-NO-CONTRACT-CHANGES (err u8001))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rebate .token-wbtc .token-usda u5976000000000 new-fee-rebate))
    (ok true)
  )
)
