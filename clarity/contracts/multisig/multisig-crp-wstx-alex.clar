...
(define-read-only (is-token-accepted (token principal))
    (or (is-eq token .yield-wstx) (is-eq token .key-wstx-alex))
)
...
(define-public (propose  
  ...
  (let 
    (
      ...
      (proposer-key-balance (unwrap-panic (contract-call? .key-wstx-alex get-balance-fixed expiry tx-sender)))
      ...
      (total-key-supply (unwrap-panic (contract-call? .key-wstx-alex get-total-supply-fixed expiry)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-x .token-alex .token-alex expiry new-fee-rate-x)))
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-y .token-alex .token-alex expiry new-fee-rate-y)))
