...
(define-read-only (is-token-accepted (token principal))
    (or (is-eq token .yield-wbtc) (is-eq token .key-wbtc-usda))
)
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-x .token-wbtc .token-wusda expiry new-fee-rate-x)))
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-y .token-wbtc .token-wusda expiry new-fee-rate-y)))
