...
(define-read-only (is-token-accepted (token <ft-trait>))
    (is-eq (contract-of token) .ytp-yield-wbtc-80875-wbtc)
)
...
(define-public (propose
  ...
  (let (
    ...
    (total-supply (* (unwrap-panic (contract-call? .ytp-yield-wbtc-80875-wbtc get-total-supply)) ONE_8))
...
(define-public (end-proposal (proposal-id uint))
  (let ((proposal (get-proposal-by-id proposal-id))
        ...
        (total-supply (unwrap-panic (contract-call? .ytp-yield-wbtc-80875-wbtc get-total-supply)))
