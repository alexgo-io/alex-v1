...
;; - CRP that mines yield-wbtc-59760 and key-usda-59760-wbtc
...
(define-read-only (is-token-accepted (vote-token <yield-token-trait>))
    (or (is-eq (contract-of vote-token) .yield-wbtc-59760) (is-eq (contract-of vote-token) .key-usda-59760-wbtc))
)
...
(define-public (propose
  ...
  (let (
    ...
    (total-yield-supply (* (unwrap-panic (contract-call? .yield-wbtc-59760 get-total-supply)) ONE_8))
    (total-key-supply (* (unwrap-panic (contract-call? .key-usda-59760-wbtc get-total-supply)) ONE_8))
