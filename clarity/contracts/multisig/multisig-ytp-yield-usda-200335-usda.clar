...
(define-read-only (is-token-accepted (token <ft-trait>))
    (is-eq (contract-of token) .ytp-yield-usda-200335-usda)
)
...
(define-public (propose
  ...
  (let (
    (proposer-balance (* (unwrap-panic (contract-call? .ytp-yield-usda-200335-usda get-balance tx-sender)) ONE_8))
    (total-supply (* (unwrap-panic (contract-call? .ytp-yield-usda-200335-usda get-total-supply)) ONE_8))
...
(define-private (execute-proposal (proposal-id uint))
  (let (
    ...
    (try! (contract-call? .yield-token-pool set-fee-rate-token .yield-usda-200335 new-fee-rate-token))
    (try! (contract-call? .yield-token-pool set-fee-rate-aytoken .yield-usda-200335 new-fee-rate-aytoken))
    (try! (contract-call? .yield-token-pool set-fee-rebate .yield-usda-200335 new-fee-rebate))
