...
(define-read-only (is-token-accepted (token principal))
    (or (is-eq token .yield-wbtc) (is-eq token .key-wbtc-alex))
)
...
(define-public (propose  
  ...
  (let 
    (
      ...
      (proposer-key-balance (unwrap-panic (contract-call? .key-wbtc-alex get-balance-fixed expiry tx-sender)))
      ...
      (total-key-supply (unwrap-panic (contract-call? .key-wbtc-alex get-total-supply-fixed expiry)))
