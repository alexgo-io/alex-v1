...
(define-read-only (is-token-accepted (token <ft-trait>))
    (is-eq (contract-of token) .ytp-yield-usda-51840-usda)
)
...
(define-public (propose
  ...
  (let (
    ...
    (total-supply (* (unwrap-panic (contract-call? .ytp-yield-usda-51840-usda get-total-supply)) ONE_8))
...
(define-private (execute-proposal (proposal-id uint))
  (let (
    ...
    (try! (contract-call? .yield-token-pool set-fee-rate-yield-token .yield-usda-51840 new-fee-rate-yield-token))
