...
(define-read-only (is-token-accepted (token <ft-trait>))
    (or (is-eq (contract-of token) .yield-wbtc-51840) (is-eq (contract-of token) .key-wbtc-51840-usda))
)
...
(define-public (propose
  ...
  (let 
    (
      ...
      (proposer-key-balance (unwrap-panic (contract-call? .key-wbtc-51840-usda get-balance tx-sender)))
      ...
      (total-key-supply (unwrap-panic (contract-call? .key-wbtc-51840-usda get-total-supply)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rate-x .token-wbtc .token-usda u5184000000000 new-fee-rate-x))
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rate-y .token-wbtc .token-usda u5184000000000 new-fee-rate-y))
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rebate .token-wbtc .token-usda u5184000000000 new-fee-rebate))
    (ok true)
  )
)
