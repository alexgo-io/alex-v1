...
(define-read-only (is-token-accepted (token <ft-trait>))
    (or (is-eq (contract-of token) .yield-usda-23040) (is-eq (contract-of token) .key-usda-23040-wbtc))
)
...
(define-public (propose
  ...
  (let 
    (
      ...
      (proposer-key-balance (unwrap-panic (contract-call? .key-usda-23040-wbtc get-balance tx-sender)))
      ...
      (total-key-supply (unwrap-panic (contract-call? .key-usda-23040-wbtc get-total-supply)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rate-x .token-usda .token-wbtc u2304000000000 new-fee-rate-x))
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rate-y .token-usda .token-wbtc u2304000000000 new-fee-rate-y))
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rebate .token-usda .token-wbtc u2304000000000 new-fee-rebate))
