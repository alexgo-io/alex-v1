...
(define-read-only (is-token-accepted (token <ft-trait>))
    (or (is-eq (contract-of token) .yield-usda-51840) (is-eq (contract-of token) .key-usda-51840-wstx))
)
...
(define-public (propose
  ...
  (let 
    (
      ...
      (proposer-key-balance (unwrap-panic (contract-call? .key-usda-51840-wstx get-balance tx-sender)))
      ...
      (total-key-supply (unwrap-panic (contract-call? .key-usda-51840-wstx get-total-supply)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rate-x .token-usda .token-wstx u5184000000000 new-fee-rate-x))
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rate-y .token-usda .token-wstx u5184000000000 new-fee-rate-y))
