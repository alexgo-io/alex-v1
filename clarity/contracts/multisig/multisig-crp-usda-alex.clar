...
(define-read-only (is-token-accepted (token principal))
    (or (is-eq token .yield-usda) (is-eq token .key-usda-alex))
)
...
(define-public (propose  
  ...
  (let 
    (
      ...
      (proposer-key-balance (unwrap-panic (contract-call? .key-usda-alex get-balance-fixed expiry tx-sender)))
      ...
      (total-key-supply (unwrap-panic (contract-call? .key-usda-alex get-total-supply-fixed expiry)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-x .token-usda .token-wbtc expiry new-fee-rate-x)))
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-y .token-usda .token-wbtc expiry new-fee-rate-y)))
