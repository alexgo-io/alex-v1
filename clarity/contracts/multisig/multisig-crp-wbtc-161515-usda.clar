...
(define-read-only (is-token-accepted (token <ft-trait>))
    (or (is-eq (contract-of token) .yield-wbtc-161515) (is-eq (contract-of token) .key-wbtc-161515-usda))
)
...
(define-public (propose
  ...
  (let 
    (
      (proposer-yield-balance (unwrap-panic (contract-call? .yield-wbtc-161515 get-balance tx-sender)))
      (proposer-key-balance (unwrap-panic (contract-call? .key-wbtc-161515-usda get-balance tx-sender)))
      (proposer-balance (+ proposer-yield-balance proposer-key-balance))
      (total-yield-supply (unwrap-panic (contract-call? .yield-wbtc-161515 get-total-supply)))
      (total-key-supply (unwrap-panic (contract-call? .key-wbtc-161515-usda get-total-supply)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rate-x .token-wbtc .token-usda u16151500000000 new-fee-rate-x))
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rate-y .token-wbtc .token-usda u16151500000000 new-fee-rate-y))
    (try! (contract-call? .collateral-rebalancing-pool set-fee-rebate .token-wbtc .token-usda u16151500000000 new-fee-rebate))
