...
(define-read-only (is-token-accepted (token principal))
    (or (is-eq token .yield-usda) (is-eq token .key-usda-wbtc))
)
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-x .token-wusda .token-wbtc expiry new-fee-rate-x)))
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-y .token-wusda .token-wbtc expiry new-fee-rate-y)))
