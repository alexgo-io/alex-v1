...
(define-read-only (is-token-accepted (token principal))
    (or (is-eq token .yield-wbtc) (is-eq token .key-wbtc-wban))
)
...
(define-public (propose  
  ...
  (let 
    (
      ...
      (proposer-key-balance (unwrap-panic (contract-call? .key-wbtc-wban get-balance-fixed expiry tx-sender)))
      ...
      (total-key-supply (unwrap-panic (contract-call? .key-wbtc-wban get-total-supply-fixed expiry)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-x .token-wbtc .token-wban expiry new-fee-rate-x)))
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-y .token-wbtc .token-wban expiry new-fee-rate-y)))
