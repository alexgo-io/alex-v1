...
(define-read-only (is-token-accepted (token principal))
    (or (is-eq token .yield-xusd) (is-eq token .key-xusd-xbtc))
)
...
(define-public (propose  
  ...
  (let 
    (
      (proposer-yield-balance (unwrap-panic (contract-call? .yield-xusd get-balance-fixed expiry tx-sender)))
      (proposer-key-balance (unwrap-panic (contract-call? .key-xusd-xbtc get-balance-fixed expiry tx-sender)))
      (proposer-balance (+ proposer-yield-balance proposer-key-balance))
      (total-yield-supply (unwrap-panic (contract-call? .yield-xusd get-total-supply-fixed expiry)))
      (total-key-supply (unwrap-panic (contract-call? .key-xusd-xbtc get-total-supply-fixed expiry)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-x .token-wxusd .token-wxbtc expiry new-fee-rate-x)))
    (as-contract (try! (contract-call? .collateral-rebalancing-pool-v1 set-fee-rate-y .token-wxusd .token-wxbtc expiry new-fee-rate-y)))
