...
(define-read-only (is-token-accepted (token <ft-trait>))
    (or (is-eq (contract-of token) .yield-wbtc-200335) (is-eq (contract-of token) .key-wbtc-200335-usda))
)
...
(define-public (propose
  ...
  (let 
    (
      (proposer-yield-balance (unwrap-panic (contract-call? .yield-wbtc-200335 get-balance tx-sender)))
      (proposer-key-balance (unwrap-panic (contract-call? .key-wbtc-200335-usda get-balance tx-sender)))
      (proposer-balance (+ proposer-yield-balance proposer-key-balance))
      (total-yield-supply (unwrap-panic (contract-call? .yield-wbtc-200335 get-total-supply)))
      (total-key-supply (unwrap-panic (contract-call? .key-wbtc-200335-usda get-total-supply)))
