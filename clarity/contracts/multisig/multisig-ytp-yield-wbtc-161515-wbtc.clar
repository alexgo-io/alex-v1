...
(define-read-only (is-token-accepted (token <ft-trait>))
    (is-eq (contract-of token) .ytp-yield-wbtc-161515-wbtc)
)
...
(define-public (propose
  ...
  (let (
    (proposer-balance (* (unwrap-panic (contract-call? .ytp-yield-wbtc-161515-wbtc get-balance tx-sender)) ONE_8))
    (total-supply (* (unwrap-panic (contract-call? .ytp-yield-wbtc-161515-wbtc get-total-supply)) ONE_8))
...
(define-public (end-proposal (proposal-id uint))
  (let ((proposal (get-proposal-by-id proposal-id))
        ...
        (total-supply (unwrap-panic (contract-call? .ytp-yield-wbtc-161515-wbtc get-total-supply)))
...
(define-private (execute-proposal (proposal-id uint))
  (let (
    ...
    (try! (contract-call? .yield-token-pool set-fee-rate-token .yield-wbtc-161515 new-fee-rate-token))
    (try! (contract-call? .yield-token-pool set-fee-rate-yield-token .yield-wbtc-161515 new-fee-rate-yield-token))
