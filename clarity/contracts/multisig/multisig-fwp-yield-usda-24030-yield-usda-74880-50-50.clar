...
(define-read-only (is-token-accepted (token <ft-trait>))
    (is-eq (contract-of token) .fwp-yield-usda-24030-yield-usda-74880-50-50)
)
...
(define-public (propose
  ...
  (let (
    (proposer-balance (unwrap-panic (contract-call? .fwp-yield-usda-24030-yield-usda-74880-50-50 get-balance tx-sender)))
    (total-supply (unwrap-panic (contract-call? .fwp-yield-usda-24030-yield-usda-74880-50-50 get-total-supply)))
...
(define-private (execute-proposal (proposal-id uint))
  (let (
    ...
    (try! (contract-call? .fixed-weight-pool set-fee-rate-x .yield-usda-24030 .yield-usda-74880 u50000000 u50000000 new-fee-rate-x))
    (try! (contract-call? .fixed-weight-pool set-fee-rate-y .yield-usda-24030 .yield-usda-74880 u50000000 u50000000 new-fee-rate-y))
...
(define-public (collect-fees-to-multisig)
    ...
    (begin
      (try! (contract-call? .fixed-weight-pool collect-fees .yield-usda-24030 .yield-usda-74880 u50000000 u50000000))
      (ok true)
    )
)
