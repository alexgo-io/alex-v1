...
(define-fungible-token auto-fwp-alex-autoalex-120)

(define-data-var contract-owner principal tx-sender)
(define-map approved-contracts principal bool)

(define-data-var token-name (string-ascii 32) "Auto ALEX / autoALEX Pool 120")
(define-data-var token-symbol (string-ascii 32) "auto-fwp-alex-autoalex-120")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-auto-fwp-alex-autoalex-120.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? auto-fwp-alex-autoalex-120 amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance auto-fwp-alex-autoalex-120 who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply auto-fwp-alex-autoalex-120))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? auto-fwp-alex-autoalex-120 amount recipient)
	)
)
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? auto-fwp-alex-autoalex-120 amount sender)
	)
)
...
(define-private (get-staking-reward (reward-cycle uint))
  (contract-call? .alex-reserve-pool get-staking-reward .fwp-alex-autoalex (get-user-id) reward-cycle)
)
...
(define-private (get-staker-at-cycle (reward-cycle uint))
  (contract-call? .alex-reserve-pool get-staker-at-cycle-or-default .fwp-alex-autoalex reward-cycle (get-user-id))
)
...
(define-private (get-user-id)
  (default-to u0 (contract-call? .alex-reserve-pool get-user-id .fwp-alex-autoalex tx-sender))
)
...
(define-private (get-reward-cycle (stack-height uint))
  (contract-call? .alex-reserve-pool get-reward-cycle .fwp-alex-autoalex stack-height)
)
...
(define-private (stake-tokens (amount-tokens uint) (lock-period uint))
  (contract-call? .alex-reserve-pool stake-tokens .fwp-alex-autoalex amount-tokens lock-period)
)
...
(define-private (claim-staking-reward (reward-cycle uint))
  (contract-call? .alex-reserve-pool claim-staking-reward .fwp-alex-autoalex reward-cycle)
)
...
(define-public (add-to-position (dx uint))
  (let
    ...
    (let
      ...
      (try! (contract-call? .fwp-alex-autoalex transfer-fixed dx sender (as-contract tx-sender) none))
      ...
      (var-set total-supply new-total-supply)
	    (try! (ft-mint? auto-fwp-alex-autoalex-120 (fixed-to-decimals (get token new-supply)) sender))
...
(define-public (claim-and-stake (reward-cycle uint))
  (let 
    (      
      ...
      (principal-balance (unwrap! (contract-call? .fwp-alex-autoalex get-balance-fixed (as-contract tx-sender)) ERR-GET-BALANCE-FIXED-FAIL))
...
(define-public (reduce-position (percent uint))
  (let 
    ...
    (as-contract (try! (contract-call? .fwp-alex-autoalex transfer-fixed reduce-principal-balance tx-sender sender none)))
    
    ;; burn pool token
    (var-set total-supply new-total-supply)
	  (try! (ft-burn? auto-fwp-alex-autoalex-120 (fixed-to-decimals reduce-supply) sender))
...
;; (contract-call? .alex-vault add-approved-token .auto-fwp-alex-autoalex-120)