...
(define-fungible-token auto-alex-v1-1)

(define-data-var contract-owner principal tx-sender)
(define-data-var token-name (string-ascii 32) "Auto ALEX")
(define-data-var token-symbol (string-ascii 32) "auto-alex-v1-1")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-auto-alex-v1-1.json"))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance auto-alex-v1-1 who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply auto-alex-v1-1))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? auto-alex-v1-1 amount recipient)
	)
)
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? auto-alex-v1-1 amount sender)
	)
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? auto-alex-v1-1 amount sender recipient))
...
;; @desc get the intrinsic value of auto-alex-v1-1
;; @desc intrinsic = next capital base of the vault / total supply of auto-alex-v1-1
...
;; @desc transfers dx to vault, stake them for 32 cycles and mints auto-alex-v1-1, the number of which is determined as % of total supply / next base
  ...
  (let
    ...
    (let
      ...
      (var-set total-supply new-total-supply)
	    (try! (ft-mint? auto-alex-v1-1 (fixed-to-decimals new-supply) sender))
...
;; claims alex for the reward-cycles and mint auto-alex-v1-1
...
(define-public (reduce-position (percent uint))
  (let 
    ...
    ;; burn pool token
    (var-set total-supply new-total-supply)
	  (try! (ft-burn? auto-alex-v1-1 (fixed-to-decimals reduce-supply) sender))
...
;; (contract-call? .alex-vault add-approved-token .auto-alex-v1-1)