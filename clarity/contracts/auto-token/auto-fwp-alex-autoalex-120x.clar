...
(define-public (add-to-position (dx uint))
    (let 
        (
          ...
          (vault (try! (contract-call? .auto-fwp-alex-autoalex-120 get-token-given-position (get token pool))))
        ...
        (as-contract (try! (contract-call? .simple-weight-pool-alex add-to-position .age000-governance-token .auto-alex .fwp-alex-autoalex-50-50-v1-01 dx (some (get dy pool)))))
        (as-contract (try! (contract-call? .auto-fwp-alex-autoalex-120 add-to-position (get token pool))))
        (map-set available-alex sender (- alex-available alex-required))
        (map-set borrowed-alex sender (+ alex-borrowed alex-required))
		    (try! (ft-mint? auto-fwp-alex-autoalex-120x (fixed-to-decimals (get token vault)) sender))
...
(define-public (reduce-position)
  (let 
    (
      ...
      (vault-reduced (as-contract (try! (contract-call? .auto-fwp-alex-autoalex-120 reduce-position share))))
      (pool-reduced (as-contract (try! (contract-call? .simple-weight-pool-alex reduce-position .age000-governance-token .auto-alex .fwp-alex-autoalex-50-50-v1-01 share))))
      ...
      (stx-to-sell (if (is-eq alex-to-buy u0) u0 (try! (contract-call? .simple-weight-pool-alex get-wstx-in-given-y-out .age000-governance-token u50000000 alex-to-buy))))
      (alex-bought (if (is-eq stx-to-sell u0) u0 (get dy (as-contract (try! (contract-call? .simple-weight-pool-alex swap-wstx-for-y .age000-governance-token u50000000 stx-to-sell (some (- alex-borrowed alex-returned))))))))
