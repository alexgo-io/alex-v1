...
(define-fungible-token auto-fwp-wstx-alex)

(define-data-var contract-owner principal tx-sender)
(define-map approved-contracts principal bool)

(define-data-var token-name (string-ascii 32) "Auto STX / ALEX Pool")
(define-data-var token-symbol (string-ascii 32) "auto-fwp-wstx-alex")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-auto-fwp-wstx-alex.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? auto-fwp-wstx-alex amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance auto-fwp-wstx-alex who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply auto-fwp-wstx-alex))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? auto-fwp-wstx-alex amount recipient)
	)
)
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? auto-fwp-wstx-alex amount sender)
	)
)
...
(define-public (add-to-position (dx uint))
  (let
    ...
    (let
	  ...
	  (try! (ft-mint? auto-fwp-wstx-alex (fixed-to-decimals (get token new-supply)) sender))
...
(define-public (reduce-position (percent uint))
  (let 
    ...
    ;; burn pool token
    (var-set total-supply new-total-supply)
	(try! (ft-burn? auto-fwp-wstx-alex (fixed-to-decimals reduce-supply) sender))
