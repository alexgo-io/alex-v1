...
(define-constant ONE_12 (pow 10 12))

;; The domain of natural exponentiation is bound by the word size and number of decimals used.
;; The largest possible result is (2^127 - 1) / 10^12, 
;; which makes the largest exponent ln((2^127 - 1) / 10^12) = 60.3986708152.
;; The smallest possible result is 10^(-12), which makes largest negative argument ln(10^(-12)) = -27.6310211159.
;; We use 69.0 and -18.0 to have some safety margin.
(define-constant MAX_NATURAL_EXPONENT (* 60 ONE_12))
(define-constant MIN_NATURAL_EXPONENT (* -27 ONE_12))

(define-constant MILD_EXPONENT_BOUND (/ (pow u2 u126) (to-uint ONE_12)))

;; Because largest exponent is 60, we start from 32 and we end at -16 because of the lowest exponent
;; The first several a_n are too large if stored as 12 decimal numbers, and could cause intermediate overflows.
;; Instead we store them as plain integers, with 0 decimals.

(define-constant x_a_list_no_deci (list 
{x_pre: 32000000000000, a_pre: 78962960182680695161, use_deci: false} ;; x0 = 2^5, a0 = e^(x0)
))

;; 16 decimal constants
(define-constant x_a_list (list 
{x_pre: 16000000000000, a_pre: 88861105205078726, use_deci: true} ;; x1 = 2^4, a1 = e^(x1)
{x_pre: 8000000000000, a_pre: 2980957987042, use_deci: true} ;; x2 = 2^3, a2 = e^(x2)
{x_pre: 4000000000000, a_pre: 5459815003314, use_deci: true} ;; x3 = 2^2, a3 = e^(x3)
{x_pre: 2000000000000, a_pre: 7389056098931, use_deci: true} ;; x4 = 2^1, a4 = e^(x4)
{x_pre: 1000000000000, a_pre: 2718281828459, use_deci: true} ;; x5 = 2^0, a5 = e^(x5)
{x_pre: 500000000000, a_pre: 1648721270700, use_deci: true} ;; x6 = 2^-1, a6 = e^(x6)
{x_pre: 250000000000, a_pre: 1284025416688, use_deci: true} ;; x7 = 2^-2, a7 = e^(x7)
{x_pre: 125000000000, a_pre: 1133148453067, use_deci: true} ;; x8 = 2^-3, a8 = e^(x8)
{x_pre: 62500000000, a_pre: 1064494458920, use_deci: true} ;; x9 = 2^-4, a9 = e^(x9)
;; {x_pre: 31250000000, a_pre: 1031743407499, use_deci: true} ;; x10 = 2^-5, a10 = e^(x10)
))


;; (define-constant x_a_list_no_deci (list 
;; {x_pre: 32000000000000, a_pre: 78962960182680695161, use_deci: true} ;; x2 = 2^5, a2 = e^(x2)
;; ))
;; ;; 12 decimal constants
;; (define-constant x_a_list (list 
;; {x_pre: 16000000000000, a_pre: 88861105205078726, use_deci: true} ;; x3 = 2^4, a3 = e^(x3)
;; {x_pre: 8000000000000, a_pre: 29809579870417, use_deci: true} ;; x4 = 2^3, a4 = e^(x4)
;; {x_pre: 4000000000000, a_pre: 545981500331, use_deci: true} ;; x5 = 2^2, a5 = e^(x5)
;; {x_pre: 2000000000000, a_pre: 73890560989, use_deci: true} ;; x6 = 2^1, a6 = e^(x6)
;; {x_pre: 1000000000000, a_pre: 27182818284, use_deci: true} ;; x7 = 2^0, a7 = e^(x7)
;; {x_pre: 500000000000, a_pre: 16487212707, use_deci: true} ;; x8 = 2^-1, a8 = e^(x8)
;; {x_pre: 250000000000, a_pre: 12840254167, use_deci: true} ;; x9 = 2^-2, a9 = e^(x9)
;; {x_pre: 125000000000, a_pre: 11331484531, use_deci: true} ;; x10 = 2^-3, a10 = e^(x10)
;; {x_pre: 62500000000, a_pre: 10644944589, use_deci: true} ;; x11 = 2^-4, a11 = e^x(11)
;; {x_pre: 31250000000, a_pre: 10317434075, use_deci: true} ;; x10 = 2^-5, a10 = e^(x10)
;; ))
...
(define-private (ln-priv (a int))
  (let
    (
      ...
      (z (/ (* (- out_a ONE_12) ONE_12) (+ out_a ONE_12)))
      (z_squared (/ (* z z) ONE_12))
...
(define-private (accumulate_division (x_a_pre (tuple (x_pre int) (a_pre int) (use_deci bool))) (rolling_a_sum (tuple (a int) (sum int))))
  (let
    ...
    (if (>= rolling_a (if use_deci a_pre (* a_pre ONE_12)))
       {a: (/ (* rolling_a (if use_deci ONE_12 1)) a_pre), sum: (+ rolling_sum x_pre)}
...
(define-private (rolling_sum_div (n int) (rolling (tuple (num int) (seriesSum int) (z_squared int))))
  (let
    (
      ...
      (next_num (/ (* rolling_num z_squared) ONE_12))
...
(define-private (accumulate_product (x_a_pre (tuple (x_pre int) (a_pre int) (use_deci bool))) (rolling_x_p (tuple (x int) (product int))))
  (let
    ...
    (if (>= rolling_x x_pre)
      {x: (- rolling_x x_pre), product: (/ (* rolling_product a_pre) (if use_deci ONE_12 1))}
...
(define-private (rolling_div_sum (n int) (rolling (tuple (term int) (seriesSum int) (x int))))
  (let
    (
      ...
      (rolling_sum (get seriesSum rolling))
      (x (get x rolling))
      (next_term (/ (/ (* rolling_term x) ONE_12) n))
...
(define-read-only (exp-fixed (x int))
  (begin
    ...
    (if (< x 0)
      ...
      ;; Fixed point division requires multiplying by ONE_12.
      (ok (/ (* ONE_12 ONE_12) (unwrap-panic (exp-pos (* -1 x)))))
      (exp-pos x)
    )
  )
)
...
(define-read-only (log-fixed (arg int) (base int))
  ...
  (let
    (
      (logBase (* (unwrap-panic (ln-priv base)) ONE_12))
      (logArg (* (unwrap-panic (ln-priv arg)) ONE_12))
   )
    (ok (/ (* logArg ONE_12) logBase))
 )
)
...
(define-read-only (ln-fixed (a int))
  (begin
    ...
    (if (< a ONE_12)
      ...
      ;; Fixed point division requires multiplying by ONE_12.
      (ok (- 0 (unwrap-panic (ln-priv (/ (* ONE_12 ONE_12) a)))))
      (ln-priv a)
   )
 )
)
...
(define-read-only (test)
  (let
    (
      ...
      ;;(arg (* 69 ONE_12))
      ;;(r (exp-pos arg))
      ;;(x_product (fold accumulate_product x_a_list {x: arg, product: ONE_12}))
  )
  ;;(ok logx-times-y)
  ;;x_product
  (ok (pow-fixed x y))
