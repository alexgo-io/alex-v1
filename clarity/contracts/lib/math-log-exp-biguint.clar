...
(define-constant MAX_NATURAL_EXPONENT (scale-up 69))
(define-constant MIN_NATURAL_EXPONENT (scale-down -18))
...
(define-read-only (accumulate_division (x_a_pre (tuple (x_pre int) (a_pre int) (use_deci bool))) (rolling_a_sum (tuple (a int) (sum int))))
  (let
    ...
    (if (>= rolling_a a_pre) ;; I think we can simply use a_pre without scaling here
