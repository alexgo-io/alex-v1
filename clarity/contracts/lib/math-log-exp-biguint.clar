...
(define-private (pow-priv (n1 {x: uint, exp: int}) (n2 {x: uint, exp: int}))
    (let
        (
            (lnx (unwrap-panic (ln-priv {x: (to-int (get x n1)), exp: (get exp n1)})))
            (logx-times-y (multiplication-with-scientific-notation (get x lnx) (get exp lnx) (to-int (get x n2)) (get exp n2)))
            (log-x (get x logx-times-y))
            (log-exp (get exp logx-times-y))
        )
        (asserts! (and (greater-than-equal-to log-x log-exp (get x MIN-NATURAL-EXPONENT) (get exp MIN-NATURAL-EXPONENT))
                    (greater-than-equal-to (get x MAX-NATURAL-EXPONENT) (get exp MAX-NATURAL-EXPONENT) log-x log-exp)) 
...
(define-private (exp-pos (n {x: int, exp: int}))
    (let
        (
            ...
            (transformed-product (transform-to-16 (get product x-product)))
            (x-out (get x x-product))
            (transformed-x-out (transform-to-16 x-out))
            (seriesSum (addition-with-scientific-notation 1 0 (get x transformed-x-out) (get exp transformed-x-out)))
            ...
            (r (multiplication-with-scientific-notation-with-precision (get x transformed-product) (get exp transformed-product) (get x sum) (get exp sum)))
        )
