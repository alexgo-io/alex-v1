...
(define-public (get-some-tokens (recipient principal))
    (begin
        ...
        (as-contract (try! (contract-call? .token-wstx transfer STX-AMOUNT tx-sender recipient)))
        (as-contract (try! (contract-call? .age000-governance-token transfer ALEX-AMOUNT tx-sender recipient)))
        (ok true)
    )
)
