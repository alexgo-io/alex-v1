...
(define-public (execute (token <ft-trait>) (amount uint))
    (let
        (   
            ...
            (swapped-token (get dx (try! (contract-call? .collateral-rebalancing-pool add-to-position-and-switch .token-usda .token-wbtc .yield-usda-51840 .key-usda-51840-wbtc gross-amount))))            
        )
