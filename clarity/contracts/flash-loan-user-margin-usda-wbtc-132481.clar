...
(define-public (execute (token <ft-trait>) (amount uint))
    (let
        (   
            ...
            (expiry (unwrap! (contract-call? .yield-wbtc-132481 get-expiry) ERR-GET-EXPIRY-FAIL-ERR))
            (ltv (try! (contract-call? .collateral-rebalancing-pool get-ltv .token-wbtc .token-usda expiry)))
            (price (try! (contract-call? .yield-token-pool get-price .yield-wbtc-132481)))
            (gross-amount (contract-call? .math-fixed-point mul-up amount (contract-call? .math-fixed-point div-down price ltv)))
            (swapped-token (get dx (try! (contract-call? .collateral-rebalancing-pool add-to-position-and-switch .token-wbtc .token-usda .yield-wbtc-132481 .key-wbtc-132481-usda gross-amount))))            
        )
        ...
        (print { object: "flash-loan-user-margin-usda-wbtc-132481", action: "execute", data: gross-amount })
        (ok true)
    )
)