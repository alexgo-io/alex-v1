...
(define-fungible-token amm-swap-pool-v1-2)
...
(define-data-var token-name (string-ascii 32) "amm-swap-pool-v1-2")
(define-data-var token-symbol (string-ascii 32) "amm-swap-pool-v1-2")
...
(define-read-only (get-overall-balance (who principal))
	(ok (ft-get-balance amm-swap-pool-v1-2 who))
)
...
(define-read-only (get-overall-supply)
	(ok (ft-get-supply amm-swap-pool-v1-2))
)
...
(define-public (transfer (token-id uint) (amount uint) (sender principal) (recipient principal))
	(let
		...
		(try! (ft-transfer? amm-swap-pool-v1-2 amount sender recipient))
...
(define-public (mint (token-id uint) (amount uint) (recipient principal))
	(begin
		...
		(try! (ft-mint? amm-swap-pool-v1-2 amount recipient))
		(try! (set-balance token-id (+ (get-balance-or-default token-id recipient) amount) recipient))
...
(define-public (burn (token-id uint) (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(try! (ft-burn? amm-swap-pool-v1-2 amount sender))
...
(define-read-only (get-overall-supply-fixed)
	(ok (decimals-to-fixed (ft-get-supply amm-swap-pool-v1-2)))
)
...
(define-read-only (get-overall-balance-fixed (who principal))
	(ok (decimals-to-fixed (ft-get-balance amm-swap-pool-v1-2 who)))
)
...
(map-set approved-contracts .amm-swap-pool-v1-2 true)