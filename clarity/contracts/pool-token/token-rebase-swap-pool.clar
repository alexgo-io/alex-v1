...
(define-fungible-token rebase-swap-pool)
...
(define-data-var token-name (string-ascii 32) "rebase-swap-pool")
(define-data-var token-symbol (string-ascii 32) "rebase-swap-pool")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-rebase-swap-pool.json"))
...
(define-read-only (get-overall-balance (who principal))
	(ok (ft-get-balance rebase-swap-pool who))
)
...
(define-read-only (get-overall-supply)
	(ok (ft-get-supply rebase-swap-pool))
)
...
(define-public (transfer (token-id uint) (amount uint) (sender principal) (recipient principal))
	(let
		...
		(try! (ft-transfer? rebase-swap-pool amount sender recipient))
...
(define-public (mint (token-id uint) (amount uint) (recipient principal))
	(begin
		...
		(try! (ft-mint? rebase-swap-pool amount recipient))
		(try! (set-balance token-id (+ (get-balance-or-default token-id recipient) amount) recipient))
...
(define-public (burn (token-id uint) (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(try! (ft-burn? rebase-swap-pool amount sender))
...
(define-read-only (get-overall-supply-fixed)
	(ok (decimals-to-fixed (ft-get-supply rebase-swap-pool)))
)
...
(define-read-only (get-overall-balance-fixed (who principal))
	(ok (decimals-to-fixed (ft-get-balance rebase-swap-pool who)))
)
...
(map-set approved-contracts .rebase-swap-pool true)