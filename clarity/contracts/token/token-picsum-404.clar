(define-non-fungible-token picsum-404-nft uint)
(define-fungible-token picsum-404)
...
(define-constant max-supply u800)
(define-constant one-8 u100000000)

(define-data-var contract-owner principal tx-sender)
(define-data-var id-nonce uint u0)
(define-map owned principal (list 800 uint))
(define-data-var available-ids (list 800 uint) (list ))
...
(define-read-only (get-owner (id uint))
    (ok (nft-get-owner? picsum-404-nft id)))
...
(define-read-only (is-id-owned-by-or-default (id uint) (owner principal))
    (match (nft-get-owner? picsum-404-nft id)
...
(define-read-only (get-balance (owner principal))
    (ok (ft-get-balance picsum-404 owner)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply picsum-404)))
...
(define-public (mint (recipient principal))
    (let (
        ...
        (try! (ft-mint? picsum-404 one-8 recipient))
        (try! (nft-mint? picsum-404-nft id recipient))
        (map-set owned recipient (unwrap-panic (as-max-len? (append (get-owned-or-default recipient) id) u800)))
        (ok (var-set id-nonce (+ id u1)))))

(define-public (mint-many (recipients (list 800 principal)))
    ...
    (begin
        ...
        (if (<= amount-or-id max-supply) ;; id transfer
            (let (
                ...
                (map-set owned recipient (unwrap-panic (as-max-len? (append owned-by-recipient amount-or-id) u800)))
                (try! (ft-transfer? picsum-404 one-8 sender recipient))
                (try! (nft-transfer? picsum-404-nft amount-or-id sender recipient))
                (ok true))
            (let (
                (balance-sender (unwrap-panic (get-balance sender)))
                (balance-recipient (unwrap-panic (get-balance recipient)))
                (check-balance (try! (ft-transfer? picsum-404 amount-or-id sender recipient)))
                ...
                (new-available-ids (if (is-eq no-to-treasury u0) (var-get available-ids) (unwrap-panic (as-max-len? (concat (var-get available-ids) ids-to-treasury) u800))))
                ...
                (map-set owned recipient (if (is-eq no-to-recipient u0) owned-by-recipient (unwrap-panic (as-max-len? (concat owned-by-recipient ids-to-recipient) u800))))
...
(define-private (nft-transfer-iter (id uint))
    (nft-transfer? picsum-404-nft id (var-get sender-temp) (var-get recipient-temp)))
...
(define-private (pop (target (list 800 uint)) (idx uint))
    (match (slice? target (+ idx u1) (len target))
        some-value (unwrap-panic (as-max-len? (concat (unwrap-panic (slice? target u0 idx)) some-value) u800))
