...
(define-public (execute-margin-usda-wbtc-16973 (the-amount uint))
...
(define-public (execute)
    (let
        (            
            (swapped-token (get dx (try! (contract-call? .collateral-rebalancing-pool add-to-position-and-switch .token-wbtc .token-usda .yield-wbtc-16973 .key-wbtc-16973-usda (var-get amount)))))            
        )
        ...
        (print { object: "flash-loan-user-margin-usda-wbtc-16973", action: "execute", data: (var-get amount) })
        (ok true)
    )
)