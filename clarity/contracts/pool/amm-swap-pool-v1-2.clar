...
(define-public (add-to-position (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dx uint) (max-dy (optional uint)))
    (let
        ...
        (try! (contract-call? token-x-trait transfer-fixed dx sender .alex-vault-v1-2 none))
        (try! (contract-call? token-y-trait transfer-fixed dy sender .alex-vault-v1-2 none))
        (map-set pools-data-map { token-x: token-x, token-y: token-y, factor: factor } pool-updated)
        (as-contract (try! (contract-call? .token-amm-swap-pool-v1-2 mint-fixed (get pool-id pool) new-supply sender)))
...
(define-public (reduce-position (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (percent uint))
    (let
        (
            ...
            (balance-x (get balance-x pool))
            (balance-y (get balance-y pool))
            (total-shares (unwrap-panic (contract-call? .token-amm-swap-pool-v1-2 get-balance-fixed (get pool-id pool) tx-sender)))
        ...
        (as-contract (try! (contract-call? .alex-vault-v1-2 transfer-ft-two token-x-trait dx token-y-trait dy sender)))
        (map-set pools-data-map { token-x: token-x, token-y: token-y, factor: factor } pool-updated)
        (as-contract (try! (contract-call? .token-amm-swap-pool-v1-2 burn-fixed (get pool-id pool) shares sender)))
...
(define-public (swap-x-for-y (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dx uint) (min-dy (optional uint)))
    (let
        ...
        (try! (contract-call? token-x-trait transfer-fixed dx sender .alex-vault-v1-2 none))
        (and (> dy u0) (as-contract (try! (contract-call? .alex-vault-v1-2 transfer-ft token-y-trait dy sender))))
        (as-contract (try! (contract-call? .alex-vault-v1-2 add-to-reserve token-x (- fee fee-rebate))))
        (map-set pools-data-map { token-x: token-x, token-y: token-y, factor: factor } pool-updated)
...
(define-public (swap-y-for-x (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dy uint) (min-dx (optional uint)))
    (let
        ...
        (try! (contract-call? token-y-trait transfer-fixed dy sender .alex-vault-v1-2 none))
        (and (> dx u0) (as-contract (try! (contract-call? .alex-vault-v1-2 transfer-ft token-x-trait dx sender))))            
        (as-contract (try! (contract-call? .alex-vault-v1-2 add-to-reserve token-y (- fee fee-rebate))))
        (map-set pools-data-map { token-x: token-x, token-y: token-y, factor: factor } pool-updated)
