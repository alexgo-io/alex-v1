...
(define-read-only (get-price (expiry uint) (yield-token principal))
    (let
        (
            ...
            (price (unwrap-panic (pow-fixed (div-down balance-y balance-x) t)))
        )      
  ...
  (begin
    ...
    (let 
      (
        ...
        (t-comp-num-uncapped (div-down ONE_8 t-comp))
        (t-comp-num (if (< t-comp-num-uncapped MILD_EXPONENT_BOUND) t-comp-num-uncapped MILD_EXPONENT_BOUND))            
        (x-pow (unwrap-panic (pow-fixed balance-x t-comp)))
        (y-pow (unwrap-panic (pow-fixed balance-y t-comp)))
        (x-dx-pow (unwrap-panic (pow-fixed (+ balance-x dx) t-comp)))
        (add-term (+ x-pow y-pow))
        (term (if (<= add-term x-dx-pow) u0 (- add-term x-dx-pow)))
        (final-term (unwrap-panic (pow-fixed term t-comp-num)))
  ...
  (begin
    ...
    (let 
      (          
        ...
        (x-pow (unwrap-panic (pow-fixed balance-x t-comp)))
        (y-pow (unwrap-panic (pow-fixed balance-y t-comp)))
        (y-dy-pow (unwrap-panic (pow-fixed (+ balance-y dy) t-comp)))
        (add-term (+ x-pow y-pow))
