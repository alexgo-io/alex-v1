...
(define-read-only (get-token-given-position (the-aytoken <yield-token-trait>) (dx uint))
    ...
    (let 
        ...
        (dy (get dy data))
        (percent-act (if (is-eq balance-aytoken u0) u0 (unwrap! (div-down balance-actual balance-aytoken) ERR-MATH-CALL)))
...
(define-read-only (get-position-given-mint (the-aytoken <yield-token-trait>) (token uint))
    ...
    (let 
        ...
        (dx (get dx data))
        (dy (get dy data))
        (percent-act (unwrap! (div-down balance-actual balance-aytoken) ERR-MATH-CALL))
        (dy-act (unwrap! (mul-down dy percent-act) ERR-MATH-CALL))
