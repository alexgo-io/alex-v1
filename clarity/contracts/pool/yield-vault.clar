...
(define-map claim-and-stake-bounty-in-fixed principal uint) ;; u100000) ;; 0.1%
(define-map claim-and-stake-bounty-max-in-fixed principal uint) ;; u1000000000) ;; 10 $ALEX

(define-read-only (get-claim-and-stake-bounty-in-fixed (token principal))
  (ok (map-get? claim-and-stake-bounty-in-fixed token))
)

(define-public (set-claim-and-stake-bounty-in-fixed (token principal) (new-claim-and-stake-bounty-in-fixed uint))
  (begin 
    (try! (check-is-owner))
    (ok (map-set claim-and-stake-bounty-in-fixed token new-claim-and-stake-bounty-in-fixed))
  )
)

(define-read-only (get-claim-and-stake-bounty-max-in-fixed (token principal))
  (ok (map-get? claim-and-stake-bounty-max-in-fixed token))
)

(define-public (set-claim-and-stake-bounty-max-in-fixed (token principal) (new-claim-and-stake-bounty-max-in-fixed uint))
  (begin 
    (try! (check-is-owner))
    (ok (map-set claim-and-stake-bounty-max-in-fixed token new-claim-and-stake-bounty-max-in-fixed))
  )
)

(define-read-only (get-activated (token principal))
  (ok (map-get? activated token))
)
