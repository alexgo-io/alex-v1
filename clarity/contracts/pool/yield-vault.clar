...
;; public functions
;;
(define-read-only (get-pool-details (staked-token principal))
    (ok (unwrap! (map-get? pools-data-map staked-token) ERR-INVALID-POOL))
)

(define-read-only (get-balance (staked-token principal))
