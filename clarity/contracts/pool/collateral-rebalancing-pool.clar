    ...
    (begin
        ...
        (let
            (
                ...
                (balance-y (get balance-y pool))
                (yield-supply (get yield-supply pool))
                (total-shares (unwrap! (contract-call? the-yield-token get-balance expiry tx-sender) ERR-GET-BALANCE-FAIL))
    ...
    (begin
        ...
        (let
            (
                ...
                (balance-y (get balance-y pool))            
                (key-supply (get key-supply pool))            
                (total-shares (unwrap! (contract-call? the-key-token get-balance expiry tx-sender) ERR-GET-BALANCE-FAIL))
                (shares (if (is-eq percent ONE_8) total-shares (mul-down total-shares percent)))
