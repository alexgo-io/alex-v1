    ...
    (if (is-eq token collateral)
        ...
        (if (is-eq token .token-wstx)
            ...
            (if (is-eq (contract-of collateral) .token-wstx)
                ...
                (ok
                    (div-down 
                        (try! (contract-call? .fixed-weight-pool get-oracle-resilient .token-wstx (contract-of collateral) u50000000 u50000000))
                        (try! (contract-call? .fixed-weight-pool get-oracle-resilient .token-wstx (contract-of token) u50000000 u50000000))
                    )
                )   
            )
        )
...
(define-public (add-to-position (token <ft-trait>) (collateral <ft-trait>) (expiry uint) (the-yield-token <sft-trait>) (the-key-token <sft-trait>) (dx uint))    
    (let
        ...
        (let
                    ...
                    (if (is-eq token-x token-y)
                        ...
                        (if (is-eq token-x .token-wstx)
                            ...
                            (if (is-eq token-y .token-wstx)
                                ...
                                (if (is-some (contract-call? .fixed-weight-pool get-pool-exists (contract-of collateral) (contract-of token) u50000000 u50000000))
            ...
            (if (is-eq token-x token-y)
                u0
                (unwrap! (contract-call? .fixed-weight-pool get-helper (contract-of collateral) (contract-of token) u50000000 u50000000 (+ dx balance-x (mul-down balance-y (try! (get-spot token collateral))))) ERR-POOL-AT-CAPACITY)
            )
...
(define-public (set-fee-to-address (token <ft-trait>) (collateral <ft-trait>) (expiry uint) (fee-to-address principal))
    (let 
        (
            (pool (try! (get-pool-details token collateral expiry)))
        )
        ...
        (let 
            (
                ...
                (dy (if (is-eq (contract-of token) (contract-of collateral))
                        ...
                        (if (is-eq (contract-of collateral) .token-wstx)
                            (try! (contract-call? .fixed-weight-pool get-y-given-wstx (contract-of token) u50000000 dx))
                            (if (is-eq (contract-of token) .token-wstx)
                                (try! (contract-call? .fixed-weight-pool get-wstx-given-y (contract-of collateral) u50000000 dx))
                                (if (is-some (contract-call? .fixed-weight-pool get-pool-exists (contract-of collateral) (contract-of token) u50000000 u50000000))
                                    (try! (contract-call? .fixed-weight-pool get-y-given-x (contract-of collateral) (contract-of token) u50000000 u50000000 dx))
                                    (try! (contract-call? .fixed-weight-pool get-x-given-y (contract-of token) (contract-of collateral) u50000000 u50000000 dx))
                                )
                            )
                        )
                    )
        ...
        (let 
            (
                ...
                (dy-to-dx (if (is-eq token-x token-y)
                            ...
                            (if (is-eq (contract-of collateral) .token-wstx)
                                (try! (contract-call? .fixed-weight-pool get-y-given-wstx (contract-of token) u50000000 dy-weighted))
                                (if (is-eq (contract-of token) .token-wstx)
                                    (try! (contract-call? .fixed-weight-pool get-wstx-given-y (contract-of collateral) u50000000 dy-weighted))
                                    (if (is-some (contract-call? .fixed-weight-pool get-pool-exists (contract-of collateral) (contract-of token) u50000000 u50000000))
                                        (try! (contract-call? .fixed-weight-pool get-y-given-x (contract-of collateral) (contract-of token) u50000000 u50000000 dy-weighted))
                                        (try! (contract-call? .fixed-weight-pool get-x-given-y (contract-of token) (contract-of collateral) u50000000 u50000000 dy-weighted))
                                    )
                                )
                            )                            
                        )
