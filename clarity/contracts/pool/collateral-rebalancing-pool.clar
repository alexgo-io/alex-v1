...
(define-private (get-token-given-position-with-spot (token principal) (collateral principal) (expiry uint) (spot uint) (dx uint))
    (let ((ltv-dy (mul-down (try! (get-ltv-with-spot token collateral expiry spot)) (mul-down (try! (oracle-instant-helper collateral token)) dx))))
        (asserts! (< block-height expiry) ERR-EXPIRY)
    ...
    (let 
        ...
        (let
            (
                ...
                (yield-amount (mul-down (mul-down gross-dx (try! (oracle-instant-helper collateral token))) ltv))
                (swapped-amount (try! (contract-call? .yield-token-pool get-x-given-y expiry-to-roll yield-token yield-amount)))
                (check2 (try! (contract-call? .yield-token-pool get-yield expiry-to-roll yield-token)))
                (ytp (try! (contract-call? .yield-token-pool get-pool-details expiry-to-roll yield-token)))
                (t (try! (contract-call? .yield-token-pool get-t expiry-to-roll (get listed ytp))))                
                (out-amount (try! (get-helper token collateral swapped-amount)))
                ;; (check (asserts! (>= yield-amount out-amount ) (err out-amount)))
                (fee (if (<= yield-amount out-amount) u0 (- yield-amount out-amount)))             
                (dx-net-fee (- dx fee))
                (gross-dx-net-fee (div-down dx-net-fee (- ONE_8 ltv)))
                (loan-amount (- gross-dx-net-fee dx-net-fee))
            ...
            (ok (- swapped-collateral-with-fee loan-amount))
        )
    )    
)
