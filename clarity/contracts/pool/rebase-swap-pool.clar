...
(use-trait ft-rebase-trait .trait-rebase-sip-010.rebase-sip-010-trait)
...
(define-public (rebase-balances (token-x principal) (token-y-trait <ft-rebase-trait>) (factor uint))
    (let 
        (    
...
(define-public (create-pool (token-x-trait <ft-trait>) (token-y-trait <ft-rebase-trait>) (factor uint) (pool-owner principal) (dx uint) (dy uint)) 
    (let
        (
...
(define-public (add-to-position (token-x-trait <ft-trait>) (token-y-trait <ft-rebase-trait>) (factor uint) (dx uint) (max-dy (optional uint)))
    (let
        (
            (token-x (contract-of token-x-trait))
            (token-y (contract-of token-y-trait))
            (pool (try! (rebase-balances token-x token-y-trait factor)))
        ...
        (map-set pools-data-map { token-x: token-x, token-y: token-y, factor: factor } pool-updated)
        (as-contract (try! (contract-call? .token-rebase-swap-pool mint-fixed (get pool-id pool) new-supply sender)))
        (print { object: "pool", action: "liquidity-added", data: pool-updated })
        (ok {supply: new-supply, dx: dx, dy: dy})
    )
)

(define-public (reduce-position (token-x-trait <ft-trait>) (token-y-trait <ft-rebase-trait>) (factor uint) (percent uint))
    (let
        (
            (token-x (contract-of token-x-trait))
            (token-y (contract-of token-y-trait))
            ...
            (balance-x (get balance-x pool))
            (balance-y (get balance-y pool))
            (total-shares (unwrap-panic (contract-call? .token-rebase-swap-pool get-balance-fixed (get pool-id pool) tx-sender)))
        ...
        (map-set pools-data-map { token-x: token-x, token-y: token-y, factor: factor } pool-updated)
        (as-contract (try! (contract-call? .token-rebase-swap-pool burn-fixed (get pool-id pool) shares sender)))
        (print { object: "pool", action: "liquidity-removed", data: pool-updated })
        (ok {dx: dx, dy: dy})
    )
)

(define-public (swap-x-for-y (token-x-trait <ft-trait>) (token-y-trait <ft-rebase-trait>) (factor uint) (dx uint) (min-dy (optional uint)))
    (let
        (
            (token-x (contract-of token-x-trait))
            (token-y (contract-of token-y-trait))
...
(define-public (swap-y-for-x (token-x-trait <ft-trait>) (token-y-trait <ft-rebase-trait>) (factor uint) (dy uint) (min-dx (optional uint)))
    (let
        (
            (token-x (contract-of token-x-trait))
            (token-y (contract-of token-y-trait))
...
;; (define-public (swap-helper (token-x principal) (token-y-trait <ft-rebase-trait>) (factor uint) (dx uint) (min-dy (optional uint)))
...
;; (define-public (swap-helper-a (token-x principal) (token-y-trait <ft-rebase-trait>) (token-z-trait <ft-rebase-trait>) (factor-x uint) (factor-y uint) (dx uint) (min-dz (optional uint)))
...
;;     (token-x principal) (token-y-trait <ft-rebase-trait>) (token-z-trait <ft-rebase-trait>) (token-w-trait <ft-rebase-trait>) 
;;     (factor-x uint) (factor-y uint) (factor-z uint)
...
;;     (token-x principal) (token-y-trait <ft-rebase-trait>) (token-z-trait <ft-rebase-trait>) (token-w-trait <ft-rebase-trait>) (token-v-trait <ft-rebase-trait>)
;;     (factor-x uint) (factor-y uint) (factor-z uint) (factor-w uint)
