...
(define-read-only (get-pool-details (token-x-trait <ft-trait>) (token-y-trait <yield-token-trait>) (weight-x uint) (weight-y uint))
    (ok (unwrap! (get-pool-exists token-x-trait token-y-trait weight-x weight-y) ERR-INVALID-POOL-ERR))
)
...
(define-read-only (get-pool-exists (token-x-trait <ft-trait>) (token-y-trait <yield-token-trait>) (weight-x uint) (weight-y uint))
...
;; @desc get-pool-exists-yt
;; @param token-x-trait; token-x
;; @param token-y-trait; token-y
;; @param weight-x; weight of token-x
;; @param weight-y; weight of token-y
;; @returns (optional (tuple))
(define-read-only (get-pool-exists-yt (token-x-trait <yield-token-trait>) (token-y-trait <ft-trait>) (weight-x uint) (weight-y uint))
    (map-get? pools-data-map { token-x: (contract-of token-x-trait), token-y: (contract-of token-y-trait), weight-x: weight-x, weight-y: weight-y }) 
)
...
;; @param weight-y; weight of token-y
;; @returns (response bool uint)
(define-public (set-oracle-average (token-x-trait <ft-trait>) (token-y-trait <yield-token-trait>) (weight-x uint) (weight-y uint) (new-oracle-average uint))
    (let
        (
            (pool (unwrap! (map-get? pools-data-map { token-x: (contract-of token-x-trait), token-y: (contract-of token-y-trait), weight-x: weight-x, weight-y: weight-y }) ERR-INVALID-POOL-ERR))
...
;; @param weight-y; weight of token-y
;; @returns (response uint uint)
(define-read-only (get-oracle-resilient (token-x-trait <ft-trait>) (token-y-trait <yield-token-trait>) (weight-x uint) (weight-y uint))
    (begin
...
;; @param weight-y; weight of token-y
;; @returns (response uint uint)
(define-read-only (get-oracle-resilient-yt (token-x-trait <yield-token-trait>) (token-y-trait <ft-trait>) (weight-x uint) (weight-y uint))
    (begin
        (if (is-some (get-pool-exists-yt token-x-trait token-y-trait weight-x weight-y))
            (let
                (
                    (pool (unwrap! (map-get? pools-data-map { token-x: (contract-of token-x-trait), token-y: (contract-of token-y-trait), weight-x: weight-x, weight-y: weight-y }) ERR-INVALID-POOL-ERR))
                )
                (asserts! (get oracle-enabled pool) ERR-ORACLE-NOT-ENABLED)
                (ok (+ (mul-down (- ONE_8 (get oracle-average pool)) (try! (get-oracle-instant-yt token-x-trait token-y-trait weight-x weight-y))) 
                       (mul-down (get oracle-average pool) (get oracle-resilient pool))))
            )
            (let
                (
...
;; @param weight-y; weight of token-y
;; @returns (response uint uint)
(define-read-only (get-oracle-instant (token-x-trait <ft-trait>) (token-y-trait <yield-token-trait>) (weight-x uint) (weight-y uint))
    (begin                
        (if (is-some (get-pool-exists token-x-trait token-y-trait weight-x weight-y))
            (let
                (
...
;; @param weight-y; weight of token-y
;; @returns (response uint uint)
(define-read-only (get-oracle-instant-yt (token-x-trait <yield-token-trait>) (token-y-trait <ft-trait>) (weight-x uint) (weight-y uint))
    (begin                
        (if (is-some (get-pool-exists-yt token-x-trait token-y-trait weight-x weight-y))
            (let
                (
...
;; @returns (response (tuple uint uint) uint)
(define-public (swap-x-for-y (token-x-trait <yield-token-trait>) (token-y-trait <ft-trait>) (weight-x uint) (weight-y uint) (dx uint) (min-dy (optional uint)))    
    (begin
                ...
                (pool-updated
                    (merge pool
                        ...
                        oracle-resilient:   (if (get oracle-enabled pool) 
                                                (try! (get-oracle-resilient-yt token-x-trait token-y-trait weight-x weight-y))
                                                u0
                                            )
                        }
                    )
...
;; @returns (response (tuple uint uint) uint)
(define-public (swap-y-for-x (token-x-trait <ft-trait>) (token-y-trait <yield-token-trait>) (weight-x uint) (weight-y uint) (dy uint) (min-dx (optional uint)))
    (begin
...
(define-read-only (get-fee-rebate (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (weight-x uint) (weight-y uint))
    (ok (get fee-rebate (unwrap! (map-get? pools-data-map { token-x: (contract-of token-x-trait), token-y: (contract-of token-y-trait), weight-x: weight-x, weight-y: weight-y }) ERR-INVALID-POOL-ERR)))
)
...
(define-read-only (get-fee-rate-x (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (weight-x uint) (weight-y uint))
    (ok (get fee-rate-x (unwrap! (map-get? pools-data-map { token-x: (contract-of token-x-trait), token-y: (contract-of token-y-trait), weight-x: weight-x, weight-y: weight-y }) ERR-INVALID-POOL-ERR)))
)
...
;; @returns (response uint uint)
(define-read-only (get-y-given-x (token-x-trait <yield-token-trait>) (token-y-trait <ft-trait>) (weight-x uint) (weight-y uint) (dx uint))
...
;; @returns (response uint uint)
(define-read-only (get-x-given-y (token-x-trait <ft-trait>) (token-y-trait <yield-token-trait>) (weight-x uint) (weight-y uint) (dy uint)) 
    (let 
        (
        (pool (unwrap! (map-get? pools-data-map { token-x: (contract-of token-x-trait), token-y: (contract-of token-y-trait), weight-x: weight-x, weight-y: weight-y }) ERR-INVALID-POOL-ERR))
        )
...
(define-public (swap (token-x-trait <ft-trait>) (token-y-trait <yield-token-trait>) (weight-x uint) (weight-y uint) (dx uint) (min-dy (optional uint)))
...
(define-read-only (get-x-y  (token-x-trait <ft-trait>) (token-y-trait <yield-token-trait>) (weight-x uint) (weight-y uint) (dx uint))
    (ok (if (is-some (get-pool-exists token-x-trait token-y-trait weight-x weight-y))
