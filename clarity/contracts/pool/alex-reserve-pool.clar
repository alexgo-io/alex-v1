...
(define-public (transfer-to-mint (usda-amount uint))
    (let
        (
            (amount-to-rebate (unwrap! (mul-down usda-amount (var-get rebate-rate)) math-call-err))
            ...
            (usda-to-alex (unwrap! (div-down usda-price alex-price) math-call-err))
            (alex-to-rebate (unwrap! (mul-down amount-to-rebate usda-to-alex) math-call-err))
        )
