...
(define-public (execute (collateral <ft-trait>) (amount uint) (memo (optional (buff 16))))
    (let
        (   
            ...
            (ltv (try! (contract-call? .collateral-rebalancing-pool-v1 get-ltv .token-wusda .age000-governance-token memo-uint)))
            (price (try! (contract-call? .yield-token-pool get-price memo-uint .yield-usda)))
            (gross-amount (mul-up amount (div-down price ltv)))            
            (minted-yield-token (get yield-token (try! (contract-call? .collateral-rebalancing-pool-v1 add-to-position .token-wusda .age000-governance-token memo-uint .yield-usda .key-usda-alex gross-amount))))
        ...
        (try! (contract-call? .swap-helper swap-helper .token-wusda .age000-governance-token swapped-token none))
