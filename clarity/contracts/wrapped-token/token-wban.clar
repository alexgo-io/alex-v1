...
(define-read-only (get-balance (account principal))
  (ok (/ (* (unwrap-panic (contract-call? .token-banana get-balance account)) (pow-decimals)) (pow u10 u6)))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (contract-call? .token-banana transfer (/ (* amount (pow u10 u6)) (pow-decimals)) sender recipient memo)
  )
)
