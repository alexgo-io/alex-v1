...
(define-fungible-token wpicsum)

(define-data-var token-name (string-ascii 32) "Wrapped Picsum 404")
(define-data-var token-symbol (string-ascii 10) "wpicsum")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wpicsum.json"))
...
(define-private (get-base-decimals)
  (contract-call? .token-picsum-404 get-decimals))

;; @desc get-balance
;; @params account
;; @returns (response uint)
(define-read-only (get-balance (account principal))
  (ok (/ (* (unwrap-panic (contract-call? .token-picsum-404 get-balance account)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (contract-call? .token-picsum-404 transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient)))
