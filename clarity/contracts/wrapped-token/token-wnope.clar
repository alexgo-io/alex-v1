...
(define-private (get-base-decimals)
  (contract-call? .token-nope get-decimals))
...
(define-read-only (get-balance (account principal))
  (ok (/ (* (unwrap-panic (contract-call? .token-nope get-balance account)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals)))))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (contract-call? .token-nope transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)
  )
)
