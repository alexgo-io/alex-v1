...
(define-public (get-some-tokens (recipient principal))
    (begin
        ...
        (and (> (var-get alex-amount) u0) (unwrap! (contract-call? .token-t-alex-v2 mint recipient (var-get alex-amount)) ERR-ALEX-TRANSFER-FAILED))
...
(define-private (mint-alex (recipient { to: principal, amount: uint }))
    (ok (and (> (get amount recipient) u0) (unwrap! (contract-call? .token-t-alex-v2 mint (get to recipient) (get amount recipient)) ERR-ALEX-TRANSFER-FAILED)))
)
