...
(define-read-only (get-total-supply)
  (ok (decimals-to-fixed (ft-get-supply key-usda-59760-wbtc)))
)
...
(define-read-only (get-balance (account principal))
  (ok (decimals-to-fixed (ft-get-balance key-usda-59760-wbtc account)))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? key-usda-59760-wbtc (fixed-to-decimals amount) sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint (recipient principal) (amount uint))
  (begin
    (asserts! (is-eq contract-caller (var-get contract-owner)) ERR-NOT-AUTHORIZED)
    (ft-mint? key-usda-59760-wbtc (fixed-to-decimals amount) recipient)
  )
)
...
(define-public (burn (sender principal) (amount uint))
  (begin
    (asserts! (is-eq contract-caller (var-get contract-owner)) ERR-NOT-AUTHORIZED)
    (ft-burn? key-usda-59760-wbtc (fixed-to-decimals amount) sender)
  )
)
