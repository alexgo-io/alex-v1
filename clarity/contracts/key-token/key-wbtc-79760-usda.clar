...
(define-read-only (get-total-supply)
  (ok (decimals-to-fixed (ft-get-supply key-wbtc-79760-usda)))
)
...
(define-read-only (get-balance (account principal))
  (ok (decimals-to-fixed (ft-get-balance key-wbtc-79760-usda account)))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? key-wbtc-79760-usda (fixed-to-decimals amount) sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint (recipient principal) (amount uint))
  (begin
    (asserts! (is-eq contract-caller (var-get contract-owner)) ERR-NOT-AUTHORIZED)
    (ft-mint? key-wbtc-79760-usda (fixed-to-decimals amount) recipient)
  )
)
...
(define-public (burn (sender principal) (amount uint))
  (begin
    (asserts! (is-eq contract-caller (var-get contract-owner)) ERR-NOT-AUTHORIZED)
    (ft-burn? key-wbtc-79760-usda (fixed-to-decimals amount) sender)
  )
)
