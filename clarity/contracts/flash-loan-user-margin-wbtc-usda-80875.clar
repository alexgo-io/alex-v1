...
(define-public (execute (token <ft-trait>) (amount uint))
    (let
        (   
            ...
            (expiry (unwrap! (contract-call? .yield-usda-80875 get-expiry) ERR-GET-EXPIRY-FAIL-ERR))
            (ltv (try! (contract-call? .collateral-rebalancing-pool get-ltv .token-usda .token-wbtc expiry)))
            (price (try! (contract-call? .yield-token-pool get-price .yield-usda-80875)))
            (gross-amount (contract-call? .math-fixed-point mul-up amount (contract-call? .math-fixed-point div-down price ltv)))
            (swapped-token (get dx (try! (contract-call? .collateral-rebalancing-pool add-to-position-and-switch .token-usda .token-wbtc .yield-usda-80875 .key-usda-80875-wbtc gross-amount))))            
        )
