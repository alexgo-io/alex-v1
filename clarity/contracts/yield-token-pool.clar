    ...
    (let
        ...
        (and (> fee-x u0) 
            (and 
                ...
                (unwrap! (contract-call? the-token transfer fee-x .alex-vault (as-contract tx-sender) none) transfer-x-failed-err)                
                ;; convert rebate amount to alex and send to fee-to-address
                (unwrap! (contract-call? .token-alex transfer (get dx (try! (contract-call? .fixed-weight-pool swap-y-for-x .token-alex the-token u50000000 u50000000 fee-x-rebate))) (as-contract tx-sender) address none) transfer-x-failed-err) 
                ;; convert the balance to stablecoin and send to reserve pool
                (unwrap! (contract-call? .token-usda transfer (get dx (try! (contract-call? .fixed-weight-pool swap-y-for-x .token-usda the-token u50000000 u50000000 fee-x-net))) (as-contract tx-sender) .alex-reserve-pool none) transfer-x-failed-err) 
            )
        )

        (and (> fee-y u0) 
            (and 
                (unwrap! (contract-call? the-aytoken transfer fee-y .alex-vault (as-contract tx-sender) none) transfer-y-failed-err)                        
                (unwrap! (contract-call? .token-alex transfer (get dx (try! (contract-call? .fixed-weight-pool swap-y-for-x .token-alex the-aytoken u50000000 u50000000 fee-y-rebate))) (as-contract tx-sender) address none) transfer-y-failed-err) 
                (unwrap! (contract-call? .token-usda transfer (get dx (try! (contract-call? .fixed-weight-pool swap-y-for-x .token-usda the-aytoken u50000000 u50000000 fee-y-net))) (as-contract tx-sender) .alex-reserve-pool none) transfer-y-failed-err) 
            )
        )          
