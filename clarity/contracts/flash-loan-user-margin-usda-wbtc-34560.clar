...
(define-public (execute-margin-usda-wbtc-34560 (the-amount uint))
...
(define-public (execute)
    (let
        (            
            (swapped-token (get dx (try! (contract-call? .collateral-rebalancing-pool add-to-position-and-switch .token-wbtc .token-usda .yield-wbtc-34560 .key-wbtc-34560-usda (var-get amount)))))            
        )
        ...
        (print { object: "flash-loan-user-margin-usda-wbtc-34560", action: "execute", data: (var-get amount) })
        (ok true)
    )
)