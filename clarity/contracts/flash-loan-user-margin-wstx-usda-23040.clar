...
(define-public (execute (token <ft-trait>) (amount uint))
    (let
        (   
            ...
            (swapped-token (get dx (try! (contract-call? .collateral-rebalancing-pool add-to-position-and-switch .token-usda .token-wstx .yield-usda-23040 .key-usda-23040-wstx gross-amount))))
        )
