...
(define-public (execute (token <ft-trait>) (amount uint))
    (let
        (   
            ...
            (expiry (unwrap! (contract-call? .yield-wbtc-161515 get-expiry) ERR-GET-EXPIRY-FAIL-ERR))
            (ltv (try! (contract-call? .collateral-rebalancing-pool get-ltv .token-wbtc .token-usda expiry)))
            (price (try! (contract-call? .yield-token-pool get-price .yield-wbtc-161515)))
            (gross-amount (contract-call? .math-fixed-point mul-up amount (contract-call? .math-fixed-point div-down price ltv)))
            (swapped-token (get dx (try! (contract-call? .collateral-rebalancing-pool add-to-position-and-switch .token-wbtc .token-usda .yield-wbtc-161515 .key-wbtc-161515-usda gross-amount))))            
        )
