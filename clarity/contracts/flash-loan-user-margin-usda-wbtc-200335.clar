...
(define-public (execute (token <ft-trait>) (amount uint))
    (let
        (   
            ...
            (expiry (unwrap! (contract-call? .yield-wbtc-200335 get-expiry) ERR-GET-EXPIRY-FAIL-ERR))
            (ltv (try! (contract-call? .collateral-rebalancing-pool get-ltv .token-wbtc .token-usda expiry)))
            (price (try! (contract-call? .yield-token-pool get-price .yield-wbtc-200335)))
            ...
            (swapped-token (get dx (try! (contract-call? .collateral-rebalancing-pool add-to-position-and-switch .token-wbtc .token-usda .yield-wbtc-200335 .key-wbtc-200335-usda gross-amount))))            
        )
