...
(define-read-only (get-total-supply)
  (ok (decimals-to-fixed (ft-get-supply yield-usda-59760)))
)
...
(define-read-only (get-balance (account principal))
  (ok (decimals-to-fixed (ft-get-balance yield-usda-59760 account)))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? yield-usda-59760 (fixed-to-decimals amount) sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint (recipient principal) (amount uint))
  (begin
    (asserts! (is-eq contract-caller (var-get contract-owner)) ERR-NOT-AUTHORIZED)
    (ft-mint? yield-usda-59760 (fixed-to-decimals amount) recipient)
  )
)
...
(define-public (burn (sender principal) (amount uint))
  (begin
    (asserts! (is-eq contract-caller (var-get contract-owner)) ERR-NOT-AUTHORIZED)
    (ft-burn? yield-usda-59760 (fixed-to-decimals amount) sender)
  )
)
...
(begin
  (try! (ft-mint? yield-usda-59760 u2000000000000 'ST1J4G6RR643BCG8G8SR6M2D9Z9KXT2NJDRK3FBTK)) ;;wallet_1
)