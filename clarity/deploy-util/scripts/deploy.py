...
for dirpath, dirnames, files in os.walk(contracts):
    for file in files:
        clarity_name = file.split('.')[0]
        res1 = subprocess.check_output(f"stx deploy_contract -x -t {os.path.join(dirpath, file)} {clarity_name} 70000 {nonce} $(cat ./deploy-keychain.json | jq -r .keyInfo.privateKey) > ../hex-files/{version_folder}/{clarity_name}.hex", shell=True)
