...
async function test_spot_trading() {
  ...
  let to_amount = parseInt(
    (await fwpGetYgivenX("token-wbtc", "token-usda", 0.5e8, 0.5e8, from_amount))
      .value.value
  );
  let exchange_rate = parseInt(
    await fwpGetYgivenX("token-wbtc", "token-usda", 0.5e8, 0.5e8, ONE_8)
  );
  await fwpSwapXforY("token-wbtc", "token-usda", 0.5e8, 0.5e8, from_amount, 0);
...
async function test_margin_trading() {
  ...
  let ltv = Number(
    (await crpGetLtv("token-usda", "token-wbtc", expiry_0)).value.value
  );
  ...
  ltv = Number(
    (await crpGetLtv("token-wbtc", "token-usda", expiry_0)).value.value
  );
...
async function get_pool_details_crp(_subset = _deploy) {
  for (const key in _subset) {
    ...
    console.log(
      "ltv: ",
      format_number(Number(ltv.value.value) / ONE_8),
      "; balance-collateral: ",
...
async function get_pool_details_ytp(_subset = _deploy) {
  for (const key in _subset) {
    ...
    console.log(
      "yield: ",
      format_number(Number(yied.value.value) / ONE_8, 8),
      "price: ",
      format_number(Number(price.value.value) / ONE_8, 8)
    );
    console.log(
